...
import { matchGoodListDsj } from 'api/couponPublish';
...
class InnerSearchableInput extends Component {
  ...
  fetch(value, callback) {
    matchGoodListDsj(value)
      .then(({dataSource}) => callback(dataSource.map(i => ({value: i.goods_id, text: i.goods_name, pic: i.pic}))));
  }
  render() {
...
SearchGoodsInput.defaultProps = {
  isFresh: false,
};

export default function SearchGoodsInput({form, isFresh, isEdit, isSelectMode, isMyVip}) {
  return (
    <FormItem label="商品名称" {...formItemLayout} required>
