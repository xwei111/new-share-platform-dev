...
export default function UserIdentity(props) {
  const { userType, couponType, isSelectMode, onModeChange, isEdit, hQBusiness, onWxModeChange, isWxMode, balance, dxMode, onDxModeChange, chooseType } = props;
  return (
    <div style={{position: 'relative'}}>
        ...
        */}
        ...
        {isSelectMode === 4 ?
            <FormItem className={styles.wxSelect} style={{marginLeft: '350px',top: '-60px'}}>
                <RadioGroup onChange={onDxModeChange} value={dxMode}>
                  <Radio value={0}>支付宝</Radio>
                  <Radio value={1}>微信公众号</Radio>
                </RadioGroup>
            </FormItem> : null
        }


        <FormItem label="" labelCol={{span: 5}} wrapperCol={{span: 19}} style={{marginTop: '50px'}}>
          <div className={styles.userInfo}>
            ...
            {userType === USER_TYPE.RETAILER.value && chooseType === 0 &&
            ...
            {userType === USER_TYPE.BRANDLER.value && chooseType === 0 &&
              <div className={styles.couponPublishMode}>
                  <RadioGroup value={isSelectMode} size="large" onChange={onModeChange}
                    disabled={hQBusiness.length === 0}>
                    <RadioButton value={0}>通用模式</RadioButton>
            ...
            {userType === USER_TYPE.MIYA.value && chooseType === 0  &&
              <div className={styles.couponPublishMode}>
                  <RadioGroup value={isSelectMode} size="large" onChange={onModeChange}
                    disabled={hQBusiness.length === 0}>
                    <RadioButton value={0}>通用模式</RadioButton>
            ...
            {userType === USER_TYPE.RETAILER.value && chooseType === 1 &&
              <div className={styles.couponPublishMode}>
                  <RadioGroup value={isSelectMode} size="large" onChange={onModeChange}
                    disabled={hQBusiness.length === 0}>
                    <RadioButton value={4}>会花定向券</RadioButton>
                  </RadioGroup>
                  {isSelectMode === 1 && <p>可在"会抢"APP中指定门店或零售商发布券</p>}
              </div>
            }

            {userType === USER_TYPE.BRANDLER.value && chooseType === 1 &&
              <div className={styles.couponPublishMode}>
                  <RadioGroup value={isSelectMode} size="large" onChange={onModeChange}
                    disabled={hQBusiness.length === 0}>
                    <RadioButton value={4}>会花定向券</RadioButton>
            ...
            {userType === USER_TYPE.MIYA.value && chooseType === 1 &&
              <div className={styles.couponPublishMode}>
                  <RadioGroup value={isSelectMode} size="large" onChange={onModeChange}
                    disabled={hQBusiness.length === 0}>
                    <RadioButton value={4}>会花定向券</RadioButton>
